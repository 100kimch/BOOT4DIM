    ...
    <q-card>
      <q-list>
        <q-collapsible icon="explore" label="개요" opened>
          <ul>
            <li v-id="!endDate">시작: {{ startDate }}</li>
            <li v-if="endDate">기간: {{ startDate + ' ~ ' + endDate }}</li>
            <li>장소: {{ project.place }}</li>
            <li v-if="project.fixedBudget">예산: {{ (parseInt(project.fixedBudget)/10000).toLocaleString() + '만원' }}</li>
  ...
  created: async function () {
    ...
    try {
      const result = await this.$gql(this.$queries.listProjects, {
        filter: {
          name: {
            contains: this.$route.params.name
          }
        }
      })
      ...
      this.$q.notify({
        color: 'negative',
        message: '프로젝트 내용을 불러오지 못했어요.'
      })
      console.error('error loadingProject(): ', e)
    }
  },
  computed: {
    startDate: function () {
      if (this.project.startDuration) {
        return this.project.startDuration.slice(0, 10)
      }
      return ''
    },
    endDate: function () {
      if (this.project.endDuration) {
